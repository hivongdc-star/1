...
// Map nội bộ nhớ user cuối cùng trong mỗi channel
const lastByChannelMap = new Map();
...
function ensurePair(users, a, b) {
  ...
  const A = users[a].relationships;
  const B = users[b].relationships;
  A.partners[b] = A.partners[b] || {
    rela: 0, daoDuyen: 0, status: "none",
    daily: { date: todayStr(), mention: 0, reply: 0 },
    chatCdAt: 0
  };
  B.partners[a] = B.partners[a] || {
    rela: 0, daoDuyen: 0, status: "none",
    daily: { date: todayStr(), mention: 0, reply: 0 },
    chatCdAt: 0
  };
...
function getRela(a, b) {
  const u = loadUsers();
  return u[a]?.relationships?.partners?.[b]?.rela || 0;
}
function getDaoDuyen(a, b) {
  const u = loadUsers();
  return u[a]?.relationships?.partners?.[b]?.daoDuyen || 0;
}
function getSpouse(a) {
  const u = loadUsers();
  const ps = u[a]?.relationships?.partners || {};
  for (const pid in ps) if (ps[pid].status === "married") return pid;
  return null;
}
function isMarried(a, b) {
  const u = loadUsers();
  return u[a]?.relationships?.partners?.[b]?.status === "married";
}
...
function eligiblePartners(a, threshold = 1000) {
  ...
  return Object.entries(ps)
    .map(([pid, cell]) => ({ partnerId: pid, value: cell.rela || 0 }))
...
function addByType(a, b, type) {
  ...
  if (type === "mention") {
    ...
    const now = Date.now();
    if ((ab.chatCdAt || 0) + CHAT_COOLDOWN_MS <= now) { ab.chatCdAt = now; delta = GAIN.chat; }
  }
  if (delta > 0) {
    ab.rela = (ab.rela || 0) + delta;
    ba.rela = (ba.rela || 0) + delta;
  }
  saveUsers(users);
}
...
function getTopPairs(limit = 10) {
  ...
  for (const a in u) {
    ...
    for (const b in ps) {
      if (a < b) {
        const cell = ps[b];
        const val = cell.status === "married" ? (cell.daoDuyen || 0) : (cell.rela || 0);
